...
// [Artem] Some custom workaround
// `geese` library requires nightly version and uses threads.
// It should be compatible with wasm, but something goes south.
// Started to get following error:
// ```
// TypeError: WebAssembly.instantiate(): Import #109 "__wbindgen_placeholder__": module is not an object or function
// ```
// After some trail and error, came up with following hack:
function hack_importObject(importObject) {
  importObject.__wbindgen_placeholder__ = {
    __wbindgen_describe: () => {
      console.log('__wbindgen_describe');
    },
    __wbindgen_throw: () => {
      const message = '__wbindgen_throw called, which is a stub!';
    ...
    __wbg_eval_17f2fea482576acf: () => {
      console.log('__wbg_eval_17f2fea482576acf');
    },
    __wbg_instanceof_DedicatedWorkerGlobalScope_9eb2c65f95cfc6a7: () => {
      console.log('__wbg_instanceof_DedicatedWorkerGlobalScope_9eb2c65f95cfc6a7');
    },
    __wbg_postMessage_54ce7f4b41ac732e: () => {
      console.log('__wbg_postMessage_54ce7f4b41ac732e');
    },
  };
  importObject.__wbindgen_externref_xform__ = {
    __wbindgen_externref_table_grow: () => {
      console.log('__wbindgen_externref_table_grow');
    },
    __wbindgen_externref_table_set_null: () => {
      console.log('__wbindgen_externref_table_set_null');
    },
  };
}

function load(wasm_path, load_cb) {
  var default_wasm_size = 2000000;
  ...
  FileLoader.load(
    ...
    function (loaded, total) {
      if (typeof load_cb == 'function') {
        load_cb(loaded / total);
      }
    },
    function (error) {
      throw error;
    ...
    function (wasm) {
      WebAssembly.compile(wasm)
        .then(function (obj) {
          add_missing_functions_stabs(obj);
          hack_importObject(importObject);
          return WebAssembly.instantiate(obj, importObject);
        })
        .then(function (obj) {
          wasm_memory = obj.exports.memory;
...
// on Google Chrome)
//
// @source: https://html-classic.itch.zone/html/9368774/dmloader.js
var FileLoader = {
  options: {
    retryCount: 4,
    retryInterval: 1000,
...
window.get_js_object = function (id) {
  return js_objects[id];
}
}());
(function () {
/**
 * https://github.com/optozorax/quad-storage/blob/master/js/quad-storage.js
 */

var ctx = null;
var memory;

var params_set_mem = function (wasm_memory, _wasm_exports) {
  memory = wasm_memory;
  ctx = {};
};

var params_register_js_plugin = function (importObject) {
...
function audio_playback_stop(playback_key) {
  let playback = playbacks.find(
    (playback) => playback.playback_key === playback_key
  );
